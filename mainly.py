import os

from subprocess import call
from tkinter import messagebox

import sys

try:
    from tkinter import*
except ImportError:
    from tkinter import*

try:
    from tkinter import ttk
    py3 = False
except ImportError:
    import tkinter.ttk as ttk
    py3 = True

def click_checkinn():
    call(["python","checkin_gui_and_program.py"])

def click_list():
    call(["python","listgui.py"])

def click_checkout():
    call(["python","checkoutgui.py"])

def click_getinfo():
    call(["python","getinfoui.py"])

class HOTEL_MANAGEMENT:
    def __init__(self):
        root = Tk()

        _bgcolor ='#d9d9d9'
        _fgcolor ='#000000'
        _compcolor ='#ffffff'
        _ana1color ='#ffffff'
        _ana2color ='#ffffff'

        font14 = "-family {Verdana} -size 15 -weight bold -slant  "\
            "roman -underline 0 -overstrike 0"
        font16 = "-family {Segoe UI} -size 40 -weight bold -slant  "\
            "roman -underline 0 -overstrike 0"
        font9 = "-family {Verdana} -size 9 -weight bold -slant  "\
            "roman -underline 0 -overstrike 0"

        root.geometry("963x600+540+110")
        root.title("DK_Python_Hotel")
        root.configure(background="brown4")
        root.configure(highlightbackground="#d9d9d9")
        root.configure(highlightcolor="black")

        self.Frame1 = Frame(root)
        self.Frame1.place(relx=0.02,rely=0.03,relheight=0.94,relwidth=0.96)
        self.Frame1.configure(relief=GROOVE)
        self.Frame1.configure(borderwidth="2")
        self.Frame1.configure(relief=GROOVE)
        self.Frame1.configure(background="coral")
        self.Frame1.configure(highlightbackground="#d9d9d9")
        self.Frame1.configure(highlightcolor="black")
        self.Frame1.configure(width=925)
        root.wm_iconbitmap(r"c:\\Tkinter_poo\\ÍCONE\\hotel.ico")

        self.Message6 = Message(self.Frame1)
        self.Message6.place(relx=0.00,rely=0.00,relheight=0.15,relwidth=1.00)
        self.Message6.configure(background="brown4")
        self.Message6.configure(font=font16)
        self.Message6.configure(foreground="white")
        self.Message6.configure(highlightbackground="#d9d9d9")
        self.Message6.configure(highlightcolor="black")
        self.Message6.configure(text='''Sistema Hotel - Seja Bem Vindo!''')
        self.Message6.configure(width=880)

        self.Button2 = Button(self.Frame1)
        self.Button2.place(relx=0.17,rely=0.17,height=86,width=566)
        self.Button2.configure(activebackground="#d9d9d9")
        self.Button2.configure(font=font14)
        self.Button2.configure(activeforeground="#000000")
        self.Button2.configure(background="green4")
        self.Button2.configure(foreground="#ffffff")
        self.Button2.configure(disabledforeground="#bfbfbf")
        self.Button2.configure(pady="0")
        self.Button2.configure(text='''1 - ENTRADA''')
        self.Button2.configure(width=566)
        self.Button2.configure(command=click_checkinn)
        

        self.Button3 = Button(self.Frame1)
        self.Button3.place(relx=0.17,rely=0.33,height=86,width=566)
        self.Button3.configure(activebackground="#d9d9d9")
        self.Button3.configure(font=font14)
        self.Button3.configure(activeforeground="#000000")
        self.Button3.configure(background="RoyalBlue1")
        self.Button3.configure(foreground="#ffffff")
        self.Button3.configure(disabledforeground="#bfbfbf")
        self.Button3.configure(pady="0")
        self.Button3.configure(text='''2 - LISTAR HÓSPEDES''')
        self.Button3.configure(width=566)
        self.Button3.configure(command=click_list)
        

        self.Button4 = Button(self.Frame1)
        self.Button4.place(relx=0.17,rely=0.49,height=86,width=566)
        self.Button4.configure(activebackground="#d9d9d9")
        self.Button4.configure(font=font14)
        self.Button4.configure(activeforeground="#000000")
        self.Button4.configure(background="Indianred3")
        self.Button4.configure(foreground="#ffffff")
        self.Button4.configure(disabledforeground="#bfbfbf")
        self.Button4.configure(pady="0")
        self.Button4.configure(text='''3 - SAÍDA''')
        self.Button4.configure(width=566)
        self.Button4.configure(command=click_checkout)
        

        self.Button5 = Button(self.Frame1)
        self.Button5.place(relx=0.17,rely=0.65,height=86,width=566)
        self.Button5.configure(activebackground="#d9d9d9")
        self.Button5.configure(font=font14)
        self.Button5.configure(foreground="#ffffff")
        self.Button5.configure(activeforeground="#000000")
        self.Button5.configure(background="Indianred4")
        self.Button5.configure(disabledforeground="#bfbfbf")
        self.Button5.configure(pady="0")
        self.Button5.configure(text='''4 - INFORMAÇÃO DO HÓSPEDE''')
        self.Button5.configure(width=566)
        self.Button5.configure(command=click_getinfo)
        

        self.Button6 = Button(self.Frame1)
        self.Button6.place(relx=0.32,rely=0.81,height=50,width=300)
        self.Button6.configure(activebackground="#d9d9d9")
        self.Button6.configure(font=font14)
        self.Button6.configure(activeforeground="#000000")
        self.Button6.configure(background="red")
        self.Button6.configure(foreground="#ffffff")
        self.Button6.configure(disabledforeground="#bfbfbf")
        self.Button6.configure(pady="0")
        self.Button6.configure(text='''SAIR''')
        self.Button6.configure(width=566)
        self.Button6.configure(command=root.destroy)

        root.mainloop()

if __name__ == '__main__':
    GUUEST=HOTEL_MANAGEMENT()